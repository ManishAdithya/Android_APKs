.class public final Lp/i;
.super Ljava/lang/Object;
.source "SourceFile"


# static fields
.field public static final p0:Landroid/util/SparseIntArray;


# instance fields
.field public A:I

.field public B:F

.field public C:I

.field public D:I

.field public E:I

.field public F:I

.field public G:I

.field public H:I

.field public I:I

.field public J:I

.field public K:I

.field public L:I

.field public M:I

.field public N:I

.field public O:I

.field public P:I

.field public Q:I

.field public R:I

.field public S:I

.field public T:F

.field public U:F

.field public V:I

.field public W:I

.field public X:I

.field public Y:I

.field public Z:I

.field public a:Z

.field public a0:I

.field public b:I

.field public b0:I

.field public c:I

.field public c0:I

.field public d:I

.field public d0:F

.field public e:I

.field public e0:F

.field public f:F

.field public f0:I

.field public g:Z

.field public g0:I

.field public h:I

.field public h0:I

.field public i:I

.field public i0:[I

.field public j:I

.field public j0:Ljava/lang/String;

.field public k:I

.field public k0:Ljava/lang/String;

.field public l:I

.field public l0:Z

.field public m:I

.field public m0:Z

.field public n:I

.field public n0:Z

.field public o:I

.field public o0:I

.field public p:I

.field public q:I

.field public r:I

.field public s:I

.field public t:I

.field public u:I

.field public v:I

.field public w:F

.field public x:F

.field public y:Ljava/lang/String;

.field public z:I


# direct methods
.method public static constructor <clinit>()V
    .locals 16

    new-instance v0, Landroid/util/SparseIntArray;

    invoke-direct {v0}, Landroid/util/SparseIntArray;-><init>()V

    sput-object v0, Lp/i;->p0:Landroid/util/SparseIntArray;

    const/16 v1, 0x2b

    const/16 v2, 0x18

    invoke-virtual {v0, v1, v2}, Landroid/util/SparseIntArray;->append(II)V

    const/16 v1, 0x2c

    const/16 v3, 0x19

    invoke-virtual {v0, v1, v3}, Landroid/util/SparseIntArray;->append(II)V

    const/16 v1, 0x2e

    const/16 v4, 0x1c

    invoke-virtual {v0, v1, v4}, Landroid/util/SparseIntArray;->append(II)V

    const/16 v1, 0x2f

    const/16 v5, 0x1d

    invoke-virtual {v0, v1, v5}, Landroid/util/SparseIntArray;->append(II)V

    const/16 v1, 0x34

    const/16 v6, 0x23

    invoke-virtual {v0, v1, v6}, Landroid/util/SparseIntArray;->append(II)V

    const/16 v1, 0x33

    const/16 v6, 0x22

    invoke-virtual {v0, v1, v6}, Landroid/util/SparseIntArray;->append(II)V

    const/4 v1, 0x4

    invoke-virtual {v0, v2, v1}, Landroid/util/SparseIntArray;->append(II)V

    const/16 v2, 0x17

    const/4 v7, 0x3

    invoke-virtual {v0, v2, v7}, Landroid/util/SparseIntArray;->append(II)V

    const/16 v8, 0x13

    const/4 v9, 0x1

    invoke-virtual {v0, v8, v9}, Landroid/util/SparseIntArray;->append(II)V

    const/16 v10, 0x3d

    const/4 v11, 0x6

    invoke-virtual {v0, v10, v11}, Landroid/util/SparseIntArray;->append(II)V

    const/16 v12, 0x3e

    const/4 v13, 0x7

    invoke-virtual {v0, v12, v13}, Landroid/util/SparseIntArray;->append(II)V

    const/16 v14, 0x1f

    const/16 v15, 0x11

    invoke-virtual {v0, v14, v15}, Landroid/util/SparseIntArray;->append(II)V

    const/16 v12, 0x20

    const/16 v3, 0x12

    invoke-virtual {v0, v12, v3}, Landroid/util/SparseIntArray;->append(II)V

    const/16 v10, 0x21

    invoke-virtual {v0, v10, v8}, Landroid/util/SparseIntArray;->append(II)V

    const/16 v8, 0xf

    const/16 v15, 0x5a

    invoke-virtual {v0, v8, v15}, Landroid/util/SparseIntArray;->append(II)V

    const/4 v8, 0x0

    const/16 v15, 0x1a

    invoke-virtual {v0, v8, v15}, Landroid/util/SparseIntArray;->append(II)V

    const/16 v8, 0x30

    invoke-virtual {v0, v8, v14}, Landroid/util/SparseIntArray;->append(II)V

    const/16 v8, 0x31

    invoke-virtual {v0, v8, v12}, Landroid/util/SparseIntArray;->append(II)V

    const/16 v8, 0x1e

    const/16 v12, 0xa

    invoke-virtual {v0, v8, v12}, Landroid/util/SparseIntArray;->append(II)V

    const/16 v8, 0x9

    invoke-virtual {v0, v5, v8}, Landroid/util/SparseIntArray;->append(II)V

    const/16 v5, 0x42

    const/16 v8, 0xd

    invoke-virtual {v0, v5, v8}, Landroid/util/SparseIntArray;->append(II)V

    const/16 v5, 0x45

    const/16 v8, 0x10

    invoke-virtual {v0, v5, v8}, Landroid/util/SparseIntArray;->append(II)V

    const/16 v5, 0x43

    const/16 v8, 0xe

    invoke-virtual {v0, v5, v8}, Landroid/util/SparseIntArray;->append(II)V

    const/16 v5, 0x40

    const/16 v8, 0xb

    invoke-virtual {v0, v5, v8}, Landroid/util/SparseIntArray;->append(II)V

    const/16 v5, 0x44

    const/16 v8, 0xf

    invoke-virtual {v0, v5, v8}, Landroid/util/SparseIntArray;->append(II)V

    const/16 v5, 0x41

    const/16 v8, 0xc

    invoke-virtual {v0, v5, v8}, Landroid/util/SparseIntArray;->append(II)V

    const/16 v5, 0x37

    const/16 v8, 0x26

    invoke-virtual {v0, v5, v8}, Landroid/util/SparseIntArray;->append(II)V

    const/16 v5, 0x25

    const/16 v8, 0x29

    invoke-virtual {v0, v8, v5}, Landroid/util/SparseIntArray;->append(II)V

    const/16 v5, 0x28

    const/16 v12, 0x27

    invoke-virtual {v0, v5, v12}, Landroid/util/SparseIntArray;->append(II)V

    const/16 v5, 0x36

    const/16 v12, 0x28

    invoke-virtual {v0, v5, v12}, Landroid/util/SparseIntArray;->append(II)V

    const/16 v5, 0x27

    const/16 v12, 0x14

    invoke-virtual {v0, v5, v12}, Landroid/util/SparseIntArray;->append(II)V

    const/16 v5, 0x35

    const/16 v12, 0x24

    invoke-virtual {v0, v5, v12}, Landroid/util/SparseIntArray;->append(II)V

    const/4 v5, 0x5

    invoke-virtual {v0, v4, v5}, Landroid/util/SparseIntArray;->append(II)V

    const/16 v4, 0x2a

    const/16 v5, 0x5b

    invoke-virtual {v0, v4, v5}, Landroid/util/SparseIntArray;->append(II)V

    const/16 v12, 0x32

    invoke-virtual {v0, v12, v5}, Landroid/util/SparseIntArray;->append(II)V

    const/16 v12, 0x2d

    invoke-virtual {v0, v12, v5}, Landroid/util/SparseIntArray;->append(II)V

    const/16 v12, 0x16

    invoke-virtual {v0, v12, v5}, Landroid/util/SparseIntArray;->append(II)V

    invoke-virtual {v0, v3, v5}, Landroid/util/SparseIntArray;->append(II)V

    invoke-virtual {v0, v7, v2}, Landroid/util/SparseIntArray;->append(II)V

    const/4 v2, 0x5

    const/16 v3, 0x1b

    invoke-virtual {v0, v2, v3}, Landroid/util/SparseIntArray;->append(II)V

    const/16 v2, 0x1e

    invoke-virtual {v0, v13, v2}, Landroid/util/SparseIntArray;->append(II)V

    const/16 v2, 0x8

    const/16 v3, 0x8

    invoke-virtual {v0, v2, v3}, Landroid/util/SparseIntArray;->append(II)V

    invoke-virtual {v0, v1, v10}, Landroid/util/SparseIntArray;->append(II)V

    const/4 v1, 0x2

    invoke-virtual {v0, v11, v1}, Landroid/util/SparseIntArray;->append(II)V

    const/16 v1, 0x16

    invoke-virtual {v0, v9, v1}, Landroid/util/SparseIntArray;->append(II)V

    const/4 v1, 0x2

    const/16 v2, 0x15

    invoke-virtual {v0, v1, v2}, Landroid/util/SparseIntArray;->append(II)V

    const/16 v1, 0x38

    invoke-virtual {v0, v1, v8}, Landroid/util/SparseIntArray;->append(II)V

    invoke-virtual {v0, v6, v4}, Landroid/util/SparseIntArray;->append(II)V

    const/16 v1, 0x11

    invoke-virtual {v0, v1, v8}, Landroid/util/SparseIntArray;->append(II)V

    const/16 v1, 0x10

    invoke-virtual {v0, v1, v4}, Landroid/util/SparseIntArray;->append(II)V

    const/16 v1, 0x47

    const/16 v2, 0x4c

    invoke-virtual {v0, v1, v2}, Landroid/util/SparseIntArray;->append(II)V

    const/16 v1, 0x19

    const/16 v2, 0x3d

    invoke-virtual {v0, v1, v2}, Landroid/util/SparseIntArray;->append(II)V

    const/16 v1, 0x1b

    const/16 v2, 0x3e

    invoke-virtual {v0, v1, v2}, Landroid/util/SparseIntArray;->append(II)V

    const/16 v1, 0x1a

    const/16 v2, 0x3f

    invoke-virtual {v0, v1, v2}, Landroid/util/SparseIntArray;->append(II)V

    const/16 v1, 0x3c

    const/16 v2, 0x45

    invoke-virtual {v0, v1, v2}, Landroid/util/SparseIntArray;->append(II)V

    const/16 v1, 0x26

    const/16 v2, 0x46

    invoke-virtual {v0, v1, v2}, Landroid/util/SparseIntArray;->append(II)V

    const/16 v1, 0xc

    const/16 v2, 0x47

    invoke-virtual {v0, v1, v2}, Landroid/util/SparseIntArray;->append(II)V

    const/16 v1, 0xa

    const/16 v2, 0x48

    invoke-virtual {v0, v1, v2}, Landroid/util/SparseIntArray;->append(II)V

    const/16 v1, 0xb

    const/16 v2, 0x49

    invoke-virtual {v0, v1, v2}, Landroid/util/SparseIntArray;->append(II)V

    const/16 v1, 0xd

    const/16 v2, 0x4a

    invoke-virtual {v0, v1, v2}, Landroid/util/SparseIntArray;->append(II)V

    const/16 v1, 0x9

    const/16 v2, 0x4b

    invoke-virtual {v0, v1, v2}, Landroid/util/SparseIntArray;->append(II)V

    return-void
.end method

.method public constructor <init>()V
    .locals 5

    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    const/4 v0, 0x0

    iput-boolean v0, p0, Lp/i;->a:Z

    const/4 v1, -0x1

    iput v1, p0, Lp/i;->d:I

    iput v1, p0, Lp/i;->e:I

    const/high16 v2, -0x40800000    # -1.0f

    iput v2, p0, Lp/i;->f:F

    const/4 v3, 0x1

    iput-boolean v3, p0, Lp/i;->g:Z

    iput v1, p0, Lp/i;->h:I

    iput v1, p0, Lp/i;->i:I

    iput v1, p0, Lp/i;->j:I

    iput v1, p0, Lp/i;->k:I

    iput v1, p0, Lp/i;->l:I

    iput v1, p0, Lp/i;->m:I

    iput v1, p0, Lp/i;->n:I

    iput v1, p0, Lp/i;->o:I

    iput v1, p0, Lp/i;->p:I

    iput v1, p0, Lp/i;->q:I

    iput v1, p0, Lp/i;->r:I

    iput v1, p0, Lp/i;->s:I

    iput v1, p0, Lp/i;->t:I

    iput v1, p0, Lp/i;->u:I

    iput v1, p0, Lp/i;->v:I

    const/high16 v4, 0x3f000000    # 0.5f

    iput v4, p0, Lp/i;->w:F

    iput v4, p0, Lp/i;->x:F

    const/4 v4, 0x0

    iput-object v4, p0, Lp/i;->y:Ljava/lang/String;

    iput v1, p0, Lp/i;->z:I

    iput v0, p0, Lp/i;->A:I

    const/4 v4, 0x0

    iput v4, p0, Lp/i;->B:F

    iput v1, p0, Lp/i;->C:I

    iput v1, p0, Lp/i;->D:I

    iput v1, p0, Lp/i;->E:I

    iput v0, p0, Lp/i;->F:I

    iput v0, p0, Lp/i;->G:I

    iput v0, p0, Lp/i;->H:I

    iput v0, p0, Lp/i;->I:I

    iput v0, p0, Lp/i;->J:I

    iput v0, p0, Lp/i;->K:I

    iput v0, p0, Lp/i;->L:I

    const/high16 v4, -0x80000000

    iput v4, p0, Lp/i;->M:I

    iput v4, p0, Lp/i;->N:I

    iput v4, p0, Lp/i;->O:I

    iput v4, p0, Lp/i;->P:I

    iput v4, p0, Lp/i;->Q:I

    iput v4, p0, Lp/i;->R:I

    iput v4, p0, Lp/i;->S:I

    iput v2, p0, Lp/i;->T:F

    iput v2, p0, Lp/i;->U:F

    iput v0, p0, Lp/i;->V:I

    iput v0, p0, Lp/i;->W:I

    iput v0, p0, Lp/i;->X:I

    iput v0, p0, Lp/i;->Y:I

    iput v0, p0, Lp/i;->Z:I

    iput v0, p0, Lp/i;->a0:I

    iput v0, p0, Lp/i;->b0:I

    iput v0, p0, Lp/i;->c0:I

    const/high16 v2, 0x3f800000    # 1.0f

    iput v2, p0, Lp/i;->d0:F

    iput v2, p0, Lp/i;->e0:F

    iput v1, p0, Lp/i;->f0:I

    iput v0, p0, Lp/i;->g0:I

    iput v1, p0, Lp/i;->h0:I

    iput-boolean v0, p0, Lp/i;->l0:Z

    iput-boolean v0, p0, Lp/i;->m0:Z

    iput-boolean v3, p0, Lp/i;->n0:Z

    iput v0, p0, Lp/i;->o0:I

    return-void
.end method


# virtual methods
.method public final a(Landroid/content/Context;Landroid/util/AttributeSet;)V
    .locals 7

    sget-object v0, Lp/q;->e:[I

    invoke-virtual {p1, p2, v0}, Landroid/content/Context;->obtainStyledAttributes(Landroid/util/AttributeSet;[I)Landroid/content/res/TypedArray;

    move-result-object p1

    invoke-virtual {p1}, Landroid/content/res/TypedArray;->getIndexCount()I

    move-result p2

    const/4 v0, 0x0

    move v1, v0

    :goto_0
    if-ge v1, p2, :cond_0

    invoke-virtual {p1, v1}, Landroid/content/res/TypedArray;->getIndex(I)I

    move-result v2

    sget-object v3, Lp/i;->p0:Landroid/util/SparseIntArray;

    invoke-virtual {v3, v2}, Landroid/util/SparseIntArray;->get(I)I

    move-result v4

    packed-switch v4, :pswitch_data_0

    packed-switch v4, :pswitch_data_1

    const/high16 v5, 0x3f800000    # 1.0f

    const-string v6, "ConstraintSet"

    packed-switch v4, :pswitch_data_2

    new-instance v4, Ljava/lang/StringBuilder;

    const-string v5, "Unknown attribute 0x"

    invoke-direct {v4, v5}, Ljava/lang/StringBuilder;-><init>(Ljava/lang/String;)V

    goto/16 :goto_1

    :pswitch_0
    const/4 v3, 0x1

    invoke-static {p0, p1, v2, v3}, Lp/m;->g(Ljava/lang/Object;Landroid/content/res/TypedArray;II)V

    goto/16 :goto_2

    :pswitch_1
    invoke-static {p0, p1, v2, v0}, Lp/m;->g(Ljava/lang/Object;Landroid/content/res/TypedArray;II)V

    goto/16 :goto_2

    :pswitch_2
    iget v3, p0, Lp/i;->W:I

    invoke-virtual {p1, v2, v3}, Landroid/content/res/TypedArray;->getInt(II)I

    move-result v2

    iput v2, p0, Lp/i;->W:I

    goto/16 :goto_2

    :pswitch_3
    iget v3, p0, Lp/i;->V:I

    invoke-virtual {p1, v2, v3}, Landroid/content/res/TypedArray;->getInt(II)I

    move-result v2

    iput v2, p0, Lp/i;->V:I

    goto/16 :goto_2

    :pswitch_4
    iget v3, p0, Lp/i;->T:F

    invoke-virtual {p1, v2, v3}, Landroid/content/res/TypedArray;->getFloat(IF)F

    move-result v2

    iput v2, p0, Lp/i;->T:F

    goto/16 :goto_2

    :pswitch_5
    iget v3, p0, Lp/i;->U:F

    invoke-virtual {p1, v2, v3}, Landroid/content/res/TypedArray;->getFloat(IF)F

    move-result v2

    iput v2, p0, Lp/i;->U:F

    goto/16 :goto_2

    :pswitch_6
    iget v3, p0, Lp/i;->x:F

    invoke-virtual {p1, v2, v3}, Landroid/content/res/TypedArray;->getFloat(IF)F

    move-result v2

    iput v2, p0, Lp/i;->x:F

    goto/16 :goto_2

    :pswitch_7
    iget v3, p0, Lp/i;->l:I

    invoke-static {p1, v2, v3}, Lp/m;->f(Landroid/content/res/TypedArray;II)I

    move-result v2

    iput v2, p0, Lp/i;->l:I

    goto/16 :goto_2

    :pswitch_8
    iget v3, p0, Lp/i;->m:I

    invoke-static {p1, v2, v3}, Lp/m;->f(Landroid/content/res/TypedArray;II)I

    move-result v2

    iput v2, p0, Lp/i;->m:I

    goto/16 :goto_2

    :pswitch_9
    iget v3, p0, Lp/i;->H:I

    invoke-virtual {p1, v2, v3}, Landroid/content/res/TypedArray;->getDimensionPixelSize(II)I

    move-result v2

    iput v2, p0, Lp/i;->H:I

    goto/16 :goto_2

    :pswitch_a
    iget v3, p0, Lp/i;->t:I

    invoke-static {p1, v2, v3}, Lp/m;->f(Landroid/content/res/TypedArray;II)I

    move-result v2

    iput v2, p0, Lp/i;->t:I

    goto/16 :goto_2

    :pswitch_b
    iget v3, p0, Lp/i;->s:I

    invoke-static {p1, v2, v3}, Lp/m;->f(Landroid/content/res/TypedArray;II)I

    move-result v2

    iput v2, p0, Lp/i;->s:I

    goto/16 :goto_2

    :pswitch_c
    iget v3, p0, Lp/i;->K:I

    invoke-virtual {p1, v2, v3}, Landroid/content/res/TypedArray;->getDimensionPixelSize(II)I

    move-result v2

    iput v2, p0, Lp/i;->K:I

    goto/16 :goto_2

    :pswitch_d
    iget v3, p0, Lp/i;->k:I

    invoke-static {p1, v2, v3}, Lp/m;->f(Landroid/content/res/TypedArray;II)I

    move-result v2

    iput v2, p0, Lp/i;->k:I

    goto/16 :goto_2

    :pswitch_e
    iget v3, p0, Lp/i;->j:I

    invoke-static {p1, v2, v3}, Lp/m;->f(Landroid/content/res/TypedArray;II)I

    move-result v2

    iput v2, p0, Lp/i;->j:I

    goto/16 :goto_2

    :pswitch_f
    iget v3, p0, Lp/i;->G:I

    invoke-virtual {p1, v2, v3}, Landroid/content/res/TypedArray;->getDimensionPixelSize(II)I

    move-result v2

    iput v2, p0, Lp/i;->G:I

    goto/16 :goto_2

    :pswitch_10
    iget v3, p0, Lp/i;->E:I

    invoke-virtual {p1, v2, v3}, Landroid/content/res/TypedArray;->getInt(II)I

    move-result v2

    iput v2, p0, Lp/i;->E:I

    goto/16 :goto_2

    :pswitch_11
    iget v3, p0, Lp/i;->i:I

    invoke-static {p1, v2, v3}, Lp/m;->f(Landroid/content/res/TypedArray;II)I

    move-result v2

    iput v2, p0, Lp/i;->i:I

    goto/16 :goto_2

    :pswitch_12
    iget v3, p0, Lp/i;->h:I

    invoke-static {p1, v2, v3}, Lp/m;->f(Landroid/content/res/TypedArray;II)I

    move-result v2

    iput v2, p0, Lp/i;->h:I

    goto/16 :goto_2

    :pswitch_13
    iget v3, p0, Lp/i;->F:I

    invoke-virtual {p1, v2, v3}, Landroid/content/res/TypedArray;->getDimensionPixelSize(II)I

    move-result v2

    iput v2, p0, Lp/i;->F:I

    goto/16 :goto_2

    :pswitch_14
    iget v3, p0, Lp/i;->b:I

    invoke-virtual {p1, v2, v3}, Landroid/content/res/TypedArray;->getLayoutDimension(II)I

    move-result v2

    iput v2, p0, Lp/i;->b:I

    goto/16 :goto_2

    :pswitch_15
    iget v3, p0, Lp/i;->c:I

    invoke-virtual {p1, v2, v3}, Landroid/content/res/TypedArray;->getLayoutDimension(II)I

    move-result v2

    iput v2, p0, Lp/i;->c:I

    goto/16 :goto_2

    :pswitch_16
    iget v3, p0, Lp/i;->w:F

    invoke-virtual {p1, v2, v3}, Landroid/content/res/TypedArray;->getFloat(IF)F

    move-result v2

    iput v2, p0, Lp/i;->w:F

    goto/16 :goto_2

    :pswitch_17
    iget v3, p0, Lp/i;->f:F

    invoke-virtual {p1, v2, v3}, Landroid/content/res/TypedArray;->getFloat(IF)F

    move-result v2

    iput v2, p0, Lp/i;->f:F

    goto/16 :goto_2

    :pswitch_18
    iget v3, p0, Lp/i;->e:I

    invoke-virtual {p1, v2, v3}, Landroid/content/res/TypedArray;->getDimensionPixelOffset(II)I

    move-result v2

    iput v2, p0, Lp/i;->e:I

    goto/16 :goto_2

    :pswitch_19
    iget v3, p0, Lp/i;->d:I

    invoke-virtual {p1, v2, v3}, Landroid/content/res/TypedArray;->getDimensionPixelOffset(II)I

    move-result v2

    iput v2, p0, Lp/i;->d:I

    goto/16 :goto_2

    :pswitch_1a
    iget v3, p0, Lp/i;->N:I

    invoke-virtual {p1, v2, v3}, Landroid/content/res/TypedArray;->getDimensionPixelSize(II)I

    move-result v2

    iput v2, p0, Lp/i;->N:I

    goto/16 :goto_2

    :pswitch_1b
    iget v3, p0, Lp/i;->R:I

    invoke-virtual {p1, v2, v3}, Landroid/content/res/TypedArray;->getDimensionPixelSize(II)I

    move-result v2

    iput v2, p0, Lp/i;->R:I

    goto/16 :goto_2

    :pswitch_1c
    iget v3, p0, Lp/i;->O:I

    invoke-virtual {p1, v2, v3}, Landroid/content/res/TypedArray;->getDimensionPixelSize(II)I

    move-result v2

    iput v2, p0, Lp/i;->O:I

    goto/16 :goto_2

    :pswitch_1d
    iget v3, p0, Lp/i;->M:I

    invoke-virtual {p1, v2, v3}, Landroid/content/res/TypedArray;->getDimensionPixelSize(II)I

    move-result v2

    iput v2, p0, Lp/i;->M:I

    goto/16 :goto_2

    :pswitch_1e
    iget v3, p0, Lp/i;->Q:I

    invoke-virtual {p1, v2, v3}, Landroid/content/res/TypedArray;->getDimensionPixelSize(II)I

    move-result v2

    iput v2, p0, Lp/i;->Q:I

    goto/16 :goto_2

    :pswitch_1f
    iget v3, p0, Lp/i;->P:I

    invoke-virtual {p1, v2, v3}, Landroid/content/res/TypedArray;->getDimensionPixelSize(II)I

    move-result v2

    iput v2, p0, Lp/i;->P:I

    goto/16 :goto_2

    :pswitch_20
    iget v3, p0, Lp/i;->u:I

    invoke-static {p1, v2, v3}, Lp/m;->f(Landroid/content/res/TypedArray;II)I

    move-result v2

    iput v2, p0, Lp/i;->u:I

    goto/16 :goto_2

    :pswitch_21
    iget v3, p0, Lp/i;->v:I

    invoke-static {p1, v2, v3}, Lp/m;->f(Landroid/content/res/TypedArray;II)I

    move-result v2

    iput v2, p0, Lp/i;->v:I

    goto/16 :goto_2

    :pswitch_22
    iget v3, p0, Lp/i;->J:I

    invoke-virtual {p1, v2, v3}, Landroid/content/res/TypedArray;->getDimensionPixelSize(II)I

    move-result v2

    iput v2, p0, Lp/i;->J:I

    goto/16 :goto_2

    :pswitch_23
    iget v3, p0, Lp/i;->D:I

    invoke-virtual {p1, v2, v3}, Landroid/content/res/TypedArray;->getDimensionPixelOffset(II)I

    move-result v2

    iput v2, p0, Lp/i;->D:I

    goto/16 :goto_2

    :pswitch_24
    iget v3, p0, Lp/i;->C:I

    invoke-virtual {p1, v2, v3}, Landroid/content/res/TypedArray;->getDimensionPixelOffset(II)I

    move-result v2

    iput v2, p0, Lp/i;->C:I

    goto/16 :goto_2

    :pswitch_25
    invoke-virtual {p1, v2}, Landroid/content/res/TypedArray;->getString(I)Ljava/lang/String;

    move-result-object v2

    iput-object v2, p0, Lp/i;->y:Ljava/lang/String;

    goto/16 :goto_2

    :pswitch_26
    iget v3, p0, Lp/i;->n:I

    invoke-static {p1, v2, v3}, Lp/m;->f(Landroid/content/res/TypedArray;II)I

    move-result v2

    iput v2, p0, Lp/i;->n:I

    goto/16 :goto_2

    :pswitch_27
    iget v3, p0, Lp/i;->o:I

    invoke-static {p1, v2, v3}, Lp/m;->f(Landroid/content/res/TypedArray;II)I

    move-result v2

    iput v2, p0, Lp/i;->o:I

    goto/16 :goto_2

    :pswitch_28
    iget v3, p0, Lp/i;->I:I

    invoke-virtual {p1, v2, v3}, Landroid/content/res/TypedArray;->getDimensionPixelSize(II)I

    move-result v2

    iput v2, p0, Lp/i;->I:I

    goto/16 :goto_2

    :pswitch_29
    iget v3, p0, Lp/i;->p:I

    invoke-static {p1, v2, v3}, Lp/m;->f(Landroid/content/res/TypedArray;II)I

    move-result v2

    iput v2, p0, Lp/i;->p:I

    goto/16 :goto_2

    :pswitch_2a
    iget v3, p0, Lp/i;->B:F

    invoke-virtual {p1, v2, v3}, Landroid/content/res/TypedArray;->getFloat(IF)F

    move-result v2

    iput v2, p0, Lp/i;->B:F

    goto/16 :goto_2

    :pswitch_2b
    iget v3, p0, Lp/i;->A:I

    invoke-virtual {p1, v2, v3}, Landroid/content/res/TypedArray;->getDimensionPixelSize(II)I

    move-result v2

    iput v2, p0, Lp/i;->A:I

    goto/16 :goto_2

    :pswitch_2c
    iget v3, p0, Lp/i;->z:I

    invoke-static {p1, v2, v3}, Lp/m;->f(Landroid/content/res/TypedArray;II)I

    move-result v2

    iput v2, p0, Lp/i;->z:I

    goto/16 :goto_2

    :pswitch_2d
    new-instance v4, Ljava/lang/StringBuilder;

    const-string v5, "unused attribute 0x"

    invoke-direct {v4, v5}, Ljava/lang/StringBuilder;-><init>(Ljava/lang/String;)V

    goto/16 :goto_1

    :pswitch_2e
    iget-boolean v3, p0, Lp/i;->g:Z

    invoke-virtual {p1, v2, v3}, Landroid/content/res/TypedArray;->getBoolean(IZ)Z

    move-result v2

    iput-boolean v2, p0, Lp/i;->g:Z

    goto/16 :goto_2

    :pswitch_2f
    invoke-virtual {p1, v2}, Landroid/content/res/TypedArray;->getString(I)Ljava/lang/String;

    move-result-object v2

    iput-object v2, p0, Lp/i;->k0:Ljava/lang/String;

    goto/16 :goto_2

    :pswitch_30
    iget-boolean v3, p0, Lp/i;->m0:Z

    invoke-virtual {p1, v2, v3}, Landroid/content/res/TypedArray;->getBoolean(IZ)Z

    move-result v2

    iput-boolean v2, p0, Lp/i;->m0:Z

    goto/16 :goto_2

    :pswitch_31
    iget-boolean v3, p0, Lp/i;->l0:Z

    invoke-virtual {p1, v2, v3}, Landroid/content/res/TypedArray;->getBoolean(IZ)Z

    move-result v2

    iput-boolean v2, p0, Lp/i;->l0:Z

    goto/16 :goto_2

    :pswitch_32
    iget v3, p0, Lp/i;->b0:I

    invoke-virtual {p1, v2, v3}, Landroid/content/res/TypedArray;->getDimensionPixelSize(II)I

    move-result v2

    iput v2, p0, Lp/i;->b0:I

    goto/16 :goto_2

    :pswitch_33
    iget v3, p0, Lp/i;->c0:I

    invoke-virtual {p1, v2, v3}, Landroid/content/res/TypedArray;->getDimensionPixelSize(II)I

    move-result v2

    iput v2, p0, Lp/i;->c0:I

    goto/16 :goto_2

    :pswitch_34
    iget v3, p0, Lp/i;->Z:I

    invoke-virtual {p1, v2, v3}, Landroid/content/res/TypedArray;->getDimensionPixelSize(II)I

    move-result v2

    iput v2, p0, Lp/i;->Z:I

    goto/16 :goto_2

    :pswitch_35
    iget v3, p0, Lp/i;->a0:I

    invoke-virtual {p1, v2, v3}, Landroid/content/res/TypedArray;->getDimensionPixelSize(II)I

    move-result v2

    iput v2, p0, Lp/i;->a0:I

    goto/16 :goto_2

    :pswitch_36
    iget v3, p0, Lp/i;->Y:I

    invoke-virtual {p1, v2, v3}, Landroid/content/res/TypedArray;->getInt(II)I

    move-result v2

    iput v2, p0, Lp/i;->Y:I

    goto/16 :goto_2

    :pswitch_37
    iget v3, p0, Lp/i;->X:I

    invoke-virtual {p1, v2, v3}, Landroid/content/res/TypedArray;->getInt(II)I

    move-result v2

    iput v2, p0, Lp/i;->X:I

    goto/16 :goto_2

    :pswitch_38
    iget v3, p0, Lp/i;->L:I

    invoke-virtual {p1, v2, v3}, Landroid/content/res/TypedArray;->getDimensionPixelSize(II)I

    move-result v2

    iput v2, p0, Lp/i;->L:I

    goto/16 :goto_2

    :pswitch_39
    iget v3, p0, Lp/i;->S:I

    invoke-virtual {p1, v2, v3}, Landroid/content/res/TypedArray;->getDimensionPixelSize(II)I

    move-result v2

    iput v2, p0, Lp/i;->S:I

    goto/16 :goto_2

    :pswitch_3a
    iget v3, p0, Lp/i;->r:I

    invoke-static {p1, v2, v3}, Lp/m;->f(Landroid/content/res/TypedArray;II)I

    move-result v2

    iput v2, p0, Lp/i;->r:I

    goto :goto_2

    :pswitch_3b
    iget v3, p0, Lp/i;->q:I

    invoke-static {p1, v2, v3}, Lp/m;->f(Landroid/content/res/TypedArray;II)I

    move-result v2

    iput v2, p0, Lp/i;->q:I

    goto :goto_2

    :pswitch_3c
    iget v3, p0, Lp/i;->o0:I

    invoke-virtual {p1, v2, v3}, Landroid/content/res/TypedArray;->getInt(II)I

    move-result v2

    iput v2, p0, Lp/i;->o0:I

    goto :goto_2

    :pswitch_3d
    iget-boolean v3, p0, Lp/i;->n0:Z

    invoke-virtual {p1, v2, v3}, Landroid/content/res/TypedArray;->getBoolean(IZ)Z

    move-result v2

    iput-boolean v2, p0, Lp/i;->n0:Z

    goto :goto_2

    :pswitch_3e
    invoke-virtual {p1, v2}, Landroid/content/res/TypedArray;->getString(I)Ljava/lang/String;

    move-result-object v2

    iput-object v2, p0, Lp/i;->j0:Ljava/lang/String;

    goto :goto_2

    :pswitch_3f
    iget v3, p0, Lp/i;->g0:I

    invoke-virtual {p1, v2, v3}, Landroid/content/res/TypedArray;->getDimensionPixelSize(II)I

    move-result v2

    iput v2, p0, Lp/i;->g0:I

    goto :goto_2

    :pswitch_40
    iget v3, p0, Lp/i;->f0:I

    invoke-virtual {p1, v2, v3}, Landroid/content/res/TypedArray;->getInt(II)I

    move-result v2

    iput v2, p0, Lp/i;->f0:I

    goto :goto_2

    :pswitch_41
    const-string v2, "CURRENTLY UNSUPPORTED"

    invoke-static {v6, v2}, Landroid/util/Log;->e(Ljava/lang/String;Ljava/lang/String;)I

    goto :goto_2

    :pswitch_42
    invoke-virtual {p1, v2, v5}, Landroid/content/res/TypedArray;->getFloat(IF)F

    move-result v2

    iput v2, p0, Lp/i;->e0:F

    goto :goto_2

    :pswitch_43
    invoke-virtual {p1, v2, v5}, Landroid/content/res/TypedArray;->getFloat(IF)F

    move-result v2

    iput v2, p0, Lp/i;->d0:F

    goto :goto_2

    :goto_1
    invoke-static {v2}, Ljava/lang/Integer;->toHexString(I)Ljava/lang/String;

    move-result-object v5

    invoke-virtual {v4, v5}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    const-string v5, "   "

    invoke-virtual {v4, v5}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {v3, v2}, Landroid/util/SparseIntArray;->get(I)I

    move-result v2

    invoke-virtual {v4, v2}, Ljava/lang/StringBuilder;->append(I)Ljava/lang/StringBuilder;

    invoke-virtual {v4}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v2

    invoke-static {v6, v2}, Landroid/util/Log;->w(Ljava/lang/String;Ljava/lang/String;)I

    :goto_2
    add-int/lit8 v1, v1, 0x1

    goto/16 :goto_0

    :cond_0
    invoke-virtual {p1}, Landroid/content/res/TypedArray;->recycle()V

    return-void

    nop

    :pswitch_data_0
    .packed-switch 0x1
        :pswitch_29
        :pswitch_28
        :pswitch_27
        :pswitch_26
        :pswitch_25
        :pswitch_24
        :pswitch_23
        :pswitch_22
        :pswitch_21
        :pswitch_20
        :pswitch_1f
        :pswitch_1e
        :pswitch_1d
        :pswitch_1c
        :pswitch_1b
        :pswitch_1a
        :pswitch_19
        :pswitch_18
        :pswitch_17
        :pswitch_16
        :pswitch_15
        :pswitch_14
        :pswitch_13
        :pswitch_12
        :pswitch_11
        :pswitch_10
        :pswitch_f
        :pswitch_e
        :pswitch_d
        :pswitch_c
        :pswitch_b
        :pswitch_a
        :pswitch_9
        :pswitch_8
        :pswitch_7
        :pswitch_6
        :pswitch_5
        :pswitch_4
        :pswitch_3
        :pswitch_2
        :pswitch_1
        :pswitch_0
    .end packed-switch

    :pswitch_data_1
    .packed-switch 0x3d
        :pswitch_2c
        :pswitch_2b
        :pswitch_2a
    .end packed-switch

    :pswitch_data_2
    .packed-switch 0x45
        :pswitch_43
        :pswitch_42
        :pswitch_41
        :pswitch_40
        :pswitch_3f
        :pswitch_3e
        :pswitch_3d
        :pswitch_3c
        :pswitch_3b
        :pswitch_3a
        :pswitch_39
        :pswitch_38
        :pswitch_37
        :pswitch_36
        :pswitch_35
        :pswitch_34
        :pswitch_33
        :pswitch_32
        :pswitch_31
        :pswitch_30
        :pswitch_2f
        :pswitch_2e
        :pswitch_2d
    .end packed-switch
.end method
